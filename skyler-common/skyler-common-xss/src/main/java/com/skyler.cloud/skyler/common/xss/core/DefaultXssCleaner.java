...
package com.skyler.cloud.skyler.common.xss.core;

import cn.hutool.core.util.CharsetUtil;
import com.skyler.cloud.skyler.common.xss.config.SkylerXssProperties;
import com.skyler.cloud.skyler.common.xss.utils.XssUtil;
...
public class DefaultXssCleaner implements XssCleaner {
	...
	private final SkylerXssProperties properties;

	public DefaultXssCleaner(SkylerXssProperties properties) {
		this.properties = properties;
	}

	private static Document.OutputSettings getOutputSettings(SkylerXssProperties properties) {
	...
	public String clean(String bodyHtml, XssType type) {
		...
		SkylerXssProperties.Mode mode = properties.getMode();
		if (SkylerXssProperties.Mode.escape == mode) {
			// html 转义
			return HtmlUtils.htmlEscape(bodyHtml, CharsetUtil.UTF_8);
		}
		else if (SkylerXssProperties.Mode.validate == mode) {
			// 校验
